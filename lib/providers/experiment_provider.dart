...
class ExperimentProvider with ChangeNotifier {
  ...
  // clearSelectedExperiment 메소드 추가
      ...
      MaterialModel(
        ...
        trainSet: [
          MaterialItem(
            ...
            type: 'train',
            naturalLanguage:
                'Find all customers who made a purchase in the last month',
            sql:
                'SELECT * FROM customers WHERE customer_id IN (SELECT customer_id FROM orders WHERE order_date > DATE_SUB(NOW(), INTERVAL 1 MONTH))',
          ),
          MaterialItem(
            id: 'train2',
            type: 'train',
      ...
      MaterialModel(
        ...
        trainSet: [
          MaterialItem(
            ...
            sql:
                'SELECT e.* FROM employees e JOIN departments d ON e.department_id = d.id JOIN salaries s ON e.id = s.employee_id WHERE s.amount > (SELECT AVG(s2.amount) FROM salaries s2 JOIN employees e2 ON s2.employee_id = e2.id WHERE e2.department_id = e.department_id)',
          ),
        ],
        testSet: [
          MaterialItem(
            ...
            sql:
                'SELECT d.name, AVG(s.amount) as avg_salary FROM departments d JOIN employees e ON d.id = e.department_id JOIN salaries s ON e.id = s.employee_id GROUP BY d.id ORDER BY avg_salary DESC',
          ),
        ],
        knowledgeSet: [
          MaterialItem(
  ...
  void selectExperiment(String experimentId) {
    _selectedExperiment = _experiments.firstWhere(
  ...
  List<Evaluation> getEvaluationsForExperiment(String experimentId) {
    return _evaluations
        .where((eval) => eval.experimentId == experimentId)
        .toList();
  }

  List<Assistant> getAssistantsForExperiment(String experimentId) {
    return _assistants
        .where((assist) => assist.experimentId == experimentId)
        .toList();
  }
  ...
  Future<void> createMaterial(
    String experimentId,
    List<MaterialItem> trainSet,
    List<MaterialItem> testSet,
    List<MaterialItem> knowledgeSet,
  ) async {
    _isLoading = true;
    notifyListeners();
  ...
  Future<void> createEvaluation(String experimentId, String materialId) async {
    ...
    try {
      ...
      final accuracy =
          material.testSet.isEmpty
              ? 0.0
              : (correctCount / material.testSet.length) * 100;
  ...
  Future<void> deployAssistant(
    ...
    try {
      ...
      final existingAssistant =
          _assistants
              .where((a) => a.name == name && a.experimentId == experimentId)
              .toList();
  ...
  Future<void> createChat(
    ...
    try {
      ...
      final aiMessage = ChatMessage(
        ...
        isUser: false,
        content: _generateAIResponse(initialMessage, assistantId),
      ...
      final newChat = Chat(
        ...
        messages: [userMessage, aiMessage], // 사용자 메시지와 AI 응답 함께 추가
        createdAt: DateTime.now(),
      );
  ...
  // AI 응답 생성 (더 자연스러운 응답)
  String _generateAIResponse(String userMessage, String assistantId) {
    // 실제 구현에서는 AI 모델을 통해 응답 생성

    // 특정 메시지 유형에 대한 응답
    if (userMessage.toLowerCase().contains('what') ||
        userMessage.toLowerCase().contains('how')) {
      final assistant = _assistants.firstWhere(
        (a) => a.id == assistantId,
        orElse: () => _assistants.first,
      );

      return "Based on the data in ${assistant.name}, I can provide the following information:\n\n"
          "The data shows ${userMessage.toLowerCase().contains('sales') ? 'sales trends over the past quarter' : 'relevant information'}. "
          "There are multiple ways to analyze this data. Would you like me to elaborate on a specific aspect?";
    } else if (userMessage.toLowerCase().contains('show') ||
        userMessage.toLowerCase().contains('get')) {
      // SQL 관련 질문에 대한 응답
      return "Here's the SQL query that can help you:\n\n"
          "```sql\nSELECT * FROM table_name\nWHERE condition = 'value'\nORDER BY column_name;\n```\n\n"
          "This will retrieve the data you're looking for. Would you like me to explain how this query works?";
    } else if (userMessage.toLowerCase().contains('thank')) {
      return "You're welcome! If you have any other questions about the data or need further assistance, feel free to ask.";
    } else {
      // 기본 응답
      return "I understand you're asking about \"${userMessage.substring(0, math.min(30, userMessage.length))}${userMessage.length > 30 ? '...' : ''}\". "
          "Based on the dataset, I can help you analyze this information. Could you specify what aspect you're most interested in?";
    }
  }
    ...
    try {
      ...
      if (index != -1) {
        ...
        final aiMessage = ChatMessage(
          id: 'msg${DateTime.now().millisecondsSinceEpoch + 1}',
          isUser: false,
          content: _generateAIResponse(content, chat.assistantId),
          timestamp: DateTime.now().add(const Duration(seconds: 1)),
        );
