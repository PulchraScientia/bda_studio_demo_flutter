...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final allAssistants = experimentProvider.getAssistantsForExperiment(
      experiment.id,
    );
                                ...
                                if (assistantNames.isNotEmpty) ...[
                                  Container(
                                    ...
                                    child: DropdownButton<String>(
                                      ...
                                      onChanged: (name) {
                                        if (name != null &&
                                            assistantsByName.containsKey(
                                              name,
                                            )) {
                                          // 이름에 해당하는 가장 최신 버전 선택
                                          final latestAssistant =
                                              assistantsByName[name]!.first;
                                          setState(() {
                                  ...
                                  Container(
                                    ...
                                    child: DropdownButton<String>(
                                      ...
                                      onChanged: (version) {
                                        if (version != null) {
                                          // 버전 문자열에서 숫자만 추출
                                          final versionNum = int.parse(
                                            version.substring(1),
                                          );
                                          // 해당 이름과 버전의 어시스턴트 찾기
                                          final selectedAssist = versions
                                              .firstWhere(
    ...
    showDialog(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: const Text(
              'Please create an assistant first by deploying an evaluation.',
            ),
            actions: [
              TextButton(
              ...
              ElevatedButton(
                onPressed: () {
                  if (_messageController.text.trim().isNotEmpty) {
                    ...
                    setState(() {
                      // 방금 생성한 채팅으로 선택
                      final chats = experimentProvider.getChatsForAssistant(
                        assistantId,
                      );
                      if (chats.isNotEmpty) {
  ...
  void _sendMessage(
    ...
    if (chats.isEmpty || !chats.any((c) => c.name == _selectedChatRoom)) {
      ...
      // 새로 생성된 채팅으로 선택 변경
      setState(() {
        final updatedChats = experimentProvider.getChatsForAssistant(
          assistantId,
        );
