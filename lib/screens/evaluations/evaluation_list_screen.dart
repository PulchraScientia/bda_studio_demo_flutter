...
class EvaluationListScreen extends StatefulWidget {
  ...
  const EvaluationListScreen({super.key, this.workspaceId, this.experimentId});
...
class _EvaluationListScreenState extends State<EvaluationListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // 해당 실험의 평가 목록 가져오기
    final allEvaluations = experimentProvider.getEvaluationsForExperiment(
      experiment.id,
    );
    ...
    final List<Evaluation> mockEvaluations =
        isWorkspaceA
            ? [
              Evaluation(
                id: 'eval1',
    ...
    for (var eval in combinedEvaluations) {
      final dateStr =
          '${eval.createdAt.year}${eval.createdAt.month.toString().padLeft(2, '0')}${eval.createdAt.day.toString().padLeft(2, '0')}';
    ...
    // 선택된 테스트 세트로 평가 필터링
    final List<Evaluation> filteredEvaluations =
        _selectedTestSet == null
            ? combinedEvaluations
    ...
    // 그래프용 데이터 생성
    final spots =
        experimentData.asMap().entries.map((entry) {
                                        ...
                                        (exp) => TableRow(
                                          children: [
                                            ...
                                            Padding(
                                              ...
                                              child: Text(
                                                '${exp['accuracy'].toStringAsFixed(1)}%',
                                              ),
                                            ),
                                          ],
                                        ),
                            ...
                            filteredEvaluations.isEmpty
                                ...
                                : Column(
                                  children:
                                      filteredEvaluations
                                          .map(
                                            (eval) => _buildEvaluationItem(
                                              title:
                                                  eval.id.contains('eval')
